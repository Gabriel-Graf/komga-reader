...
- **Data (local-only):** Room table `novel_bookmark` (`NovelBookmarkEntity`, `NovelBookmarkDao`,
  domain `NovelBookmarkRepository`, `RoomNovelBookmarkRepository`); `AppDatabase` 18 → 19 with
  `MIGRATION_18_19` (renumbered when merged onto main, which already used 17→18 for `reading_session`).
  Deliberately **not** registered with the sync queue (no server has per-word bookmarks).
