...
- **Very low entropy.** Zero `TODO`/`FIXME`/`HACK` in production code; only 2 `@Deprecated`
  symbols (the documented `RefreshScheduler`); only a handful of `!!` force‑unwraps outside
  tests.
...
9. Remove the deprecated `RefreshScheduler` once the device‑managed path is the sole path.
