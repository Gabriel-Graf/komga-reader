...
fun PagedReaderScreen(
    ...
    val pagerState = rememberPagerState(initialPage = state.initialPage) { pageCount }
        ...
        Box(Modifier.fillMaxSize()) {
            HorizontalPager(
                ...
                modifier = Modifier
                    ...
                    .padding(if (state.chromeVisible) padding else PaddingValues(0.dp)),
            ) { pageIndex ->
                val pageRef = pages[pageIndex]
                val request = remember(pageRef.url, state.apiKey) {
                    ImageRequest.Builder(ctx)
                        .data(pageRef.url)
                        .addHeader("X-API-Key", state.apiKey)
                        .crossfade(false)
                        .build()
                }
                AsyncImage(
