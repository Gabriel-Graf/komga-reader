...
/**
 * Proves MIGRATION_17_18 is non-destructive: a v17 database with an existing row in
 * `read_progress` is opened through the real migration. Afterwards (a) the old row must be
 * intact (no wipe) and (b) the new `novel_bookmark` table must be usable. `exportSchema = false`
 * → no MigrationTestHelper; instead the v17 DB is created raw and then opened via Room with the
 * migration.
 *
...
class NovelBookmarkMigrationTest {
    ...
    @Test
    fun migration17to18_legtNovelBookmarkAn_ohneBestandsdatenZuLoeschen() = runBlocking {
        // 1. v17-DB roh erzeugen: volles v17-Schema + eine read_progress-Bestandszeile.
        createV17DatabaseWithReadProgressRow()

        // 2. Über Room mit der echten Migration öffnen (Ziel-Version = 18).
